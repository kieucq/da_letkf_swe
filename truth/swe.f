...
c
      ...
      open (22,file='bvortex_truth.dat',status='old')                              
      open (28,file='truth.dat',form='unformatted',
...
11100    zo(i,j)= z(i,j)                                                        
      ...
      if (ijk.eq.0  ) then                                                         
       ...
       jint      = ifix(toint*3600./abs(dt))                                              
       if (jint.lt.1) jint = 1                                                    
       jout      = jin+jint                                                           
       jstop     = tfcst/toint+0.5                                                   
      endif            
      print*,'truth.exe: intialized option ijk =',ijk
      print*,'truth.exe: first forecast time index jin =',jin
      print*,'truth.exe: end forecast time index jout =',jout
      print*,'truth.exe: time step dt  =',dt,'s'
      print*,'truth.exe: # of DA cycle jstop  =',jstop
      print*,'truth.exe: # of timesteps for each DA cycle jint =',jint
      print*,'truth.exe: DA window toint =',toint,'h'
c                                                                               
...
1006     format(2x,'after invart',5x,'output step number = ',i5)
        ...
        if (nprint.eq.0) then
            ...
            ofile   = 'tru_00:00:00.dat'
            print*,kk,time,jin,jout,jint,jstop
            print*,iday,ihour,imin
            if (iday.lt.10) then
