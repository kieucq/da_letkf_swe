...
c
      ...
      open (22,file='ana.dat',status='old')                              
      open (28,file='swe.dat',form='unformatted',
...
11100    zo(i,j)= z(i,j)                                                        
      ...
      print*,'model.exe: jin    =',jin
      print*,'model.exe: jout   =',jout
      print*,'model.exe: nt     =',nt
      print*,'model.exe: jstop  =',jstop
      print*,'model.exe: jint   =',jint
c                                                                               
...
1006     format(2x,'after invart',5x,'output step number = ',i5)
        ...
        elseif (iday.lt.100) then
          ...
          print*,'model.exe: day string length is too long...stop'
          stop
        endif
        if (ihour.lt.10) then
        ...
        elseif (iday.lt.100) then
          ...
          print*,'model.exe: hour string length is too long...stop'
          stop
        endif
        if (imin.lt.10) then
        ...
        elseif (iday.lt.100) then
          ...
          print*,'model.exe: minute string length is too long...stop'
          stop
        endif 
        if (debug.ge.1) print*,'model.exe: output is: ',ofile(1:20)
        open(29,file=ofile,status='unknown')
        write(29,'(6F13.6)')((u(i,j),i=1,lg),j=1,mg)
        write(29,'(6F13.6)')((v(i,j),i=1,lg),j=1,mg)
        write(29,'(6F13.6)')((z(i,j),i=1,lg),j=1,mg)
        close(29)
        if (nprint.gt.0.and.nprint.le.nt.and.restart_flag.eq.1) then 
         ofile   = 'bgd_t000.dat'
         if (nprint.lt.10) then
          write(ofile(8:8),'(1I1)')nprint
         elseif (nprint.lt.100) then
          write(ofile(7:8),'(1I2)')nprint
         elseif (nprint.lt.1000) then
          write(ofile(6:8),'(1I3)')nprint
         else
          print*,'model.exe: too many slices in 4d window...stop'
          stop
         endif
         open(29,file=ofile,status='unknown')
         write(29,'(6F13.6)')((u(i,j),i=1,lg),j=1,mg)
         ...
         write(29,'(6F13.6)')((z(i,j),i=1,lg),j=1,mg)
         close(29)
         if (nprint.eq.nt) restart_flag=0
         nprint = nprint + 1
        else
         nprint = 0
        endif
...
c                                                                                
     ...
     &           slat,dt,dy,restart,t_window,dt_window)
