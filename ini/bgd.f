...
c                                                                               
     ...
     &              slat,   dphi                                                
      ...
      common /a13/  restart,t_window,dt_window
      common /a14/  nmember
      integer irec,debug,restart_flag,iday,ihour,imin,out_flag
      integer nprint
...
101   continue
      ...
      elseif (ie.lt.1000) then
       ...
       print*,'colds.exe: too many members...stop'
       stop
      endif
      open (22,file=ifile,status='old')
      open (28,file='bgd.dat',form='unformatted',
...
11100    zo(i,j)= z(i,j)                                                        
      ...
      out_flag   = nint(restart/toint)
      print*,'colds.exe: jin    =',jin
      print*,'colds.exe: jout   =',jout
      print*,'colds.exe: nt     =',nt
      print*,'colds.exe: jstop  =',jstop
      print*,'colds.exe: jint   =',jint
      print*,'colds.exe: out_flag =',out_flag
c                                                                               
...
1006     format(2x,'after invart',5x,'output step number = ',i5)
        ...
        if (mod(kk,out_flag).eq.0) nprint = 1
        if (kk.eq.1)               nprint = 2
        if (nprint.gt.0.and.nprint.le.nt) then
         ...
         if (iday.lt.10) then
          write(ofile(10:10),'(1I1)')iday
         elseif (iday.lt.100) then
          write(ofile(9:10),'(1I2)')iday
         else
          print*,'colds.exe: day string length is too long...stop'
          stop
         endif
         if (ihour.lt.10) then
          write(ofile(13:13),'(1I1)')ihour
         elseif (iday.lt.100) then
          write(ofile(12:13),'(1I2)')ihour
         else
          print*,'colds.exe: hour string length is too long...stop'
          stop
         endif
         if (imin.lt.10) then
          write(ofile(16:16),'(1I1)')imin
         elseif (iday.lt.100) then
          write(ofile(15:16),'(1I2)')imin
         else
          print*,'colds.exe: minute string length is too long...stop'
          stop
         endif 
         if (debug.ge.1) print*,'colds.exe: open output is:',ofile(1:20)
         open(29,file=ofile,status='unknown')
         write(29,'(6F13.6)')((u(i,j),i=1,lg),j=1,mg)
         write(29,'(6F13.6)')((v(i,j),i=1,lg),j=1,mg)
         write(29,'(6F13.6)')((z(i,j),i=1,lg),j=1,mg)
...
c
      call input_namelist(debug,l,m,tfcst,slat,dt,dy,
     &restart,t_window,dt_window,nmember)
      toint     = dt_window/3600.
      dphi      = dy/111e3
      ijk       = 1
      nohist    = 1
      times     = 0
      ...
      if (debug.eq.1) then
       ...
       print*,'swe.exe: output interval toint  =',toint
       print*,'swe.exe: domain grid dx         =',dy,dphi
       print*,'swe.exe: starting lat slat      =',slat
       print*,'swe.exe: t_window is            =',t_window
       print*,'swe.exe: dt_window is           =',dt_window
       print*,'swe.exe: restart                =',restart
       read*
      endif 
c                                                                               
      m1        = m-1                                                                  
...
c                                                                                
     ...
     &           slat,dt,dy,restart,t_window,dt_window,ne)
